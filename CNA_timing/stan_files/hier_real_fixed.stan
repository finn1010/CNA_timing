...
transformed parameters {
    ...
    for (j in 1:sum(J)) {       //loop for number if CNA events
        ...
        if (type == 1) {
            cache_theta[j] = ss_cnloh_evln(t[j], age[j], mu[p], mu[p]);
        } else if (type == 2) {
            cache_theta[j] = ss_tri_evln(t[j], age[j], mu[p], mu[p]);
        } else if (type == 3) {
            cache_theta[j] = ss_tet_evln(t[j], age[j], mu[p], mu[p]);
        } else if (type == 4) {
            cache_theta[j] = dip_cnloh_evln(t[j], age[j], mu[p], mu[p], [0.5, 0, 0.5]');
        } else if (type == 5) {
            cache_theta[j] = dip_tri_evln(t[j], age[j], mu[p], mu[p], [0.5, 0, 0.5]');
        } else if (type == 6) {
            cache_theta[j] = dip_tet_evln(t[j], age[j], mu[p], mu[p], [0.5, 0, 0.5]');
        }
    }
}
