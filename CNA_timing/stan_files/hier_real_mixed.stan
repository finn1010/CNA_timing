\\hierarchical model for real data
...
model {
    mu_mean ~ normal(-5.3, 0.3);
    mu_sdd  ~ normal(0, 0.5);

    for (p in 1:P) {
        mu_raw[p]    ~ normal(0, 1);
        gamma_raw[p] ~ normal(1, 0.25);
    }
    ...
    for (j in 1:sum(J)) {
        ...
        if (type[j] == 4) {
            cache_theta_loh = dip_cnloh_evln(t[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5,0,0.5]');
            cache_theta_loh /= sum(cache_theta_loh);  

        } else if (type[j] == 5) {
            cache_theta_tri = dip_tri_evln(t[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5,0,0.5]');
            cache_theta_tri /= sum(cache_theta_tri); 

        } else if (type[j] == 6) {
            cache_theta_tet = dip_tet_evln(t[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5,0,0.5]');
...
generated quantities {
    ...
    for (j in 1:sum(J)) {
        ...
        if (type[j] == 4) {
            cache_theta_loh = dip_cnloh_evln(t_pred[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5, 0, 0.5]');
            cache_theta_loh /= sum(cache_theta_loh);
        } else if (type[j] == 5) {
            cache_theta_tri = dip_tri_evln(t_pred[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5, 0, 0.5]');
            cache_theta_tri /= sum(cache_theta_tri);
        } else if (type[j] == 6) {
            cache_theta_tet = dip_tet_evln(t_pred[j], age[j], mu[patient_idx[j]], gamma[patient_idx[j]], [0.5, 0, 0.5]');
            cache_theta_tet /= sum(cache_theta_tet);
        }
