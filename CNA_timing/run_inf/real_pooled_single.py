...
#     plt.hist(cna_beta_values, fbins=13, alpha=0.7, color='blue', edgecolor='black', density=True)
#     plt.xlabel("Fraction of alleles methylated", fontsize=14.5)
#     plt.ylabel("Probability Density", fontsize=14.5)
# #     plt.title(f"CNA {cna_number} (sample_chr: {sample_chr_id})")
...
model = cmdstanpy.CmdStanModel(stan_file='real_pooled_single.stan')
data = {
    'K': K, 
    'J': extracted_data['cna_per_patient'],
    'P': max(idx),
...
az_fit.to_netcdf(f"real_pooled_single.nc")

with open(f'real_pooled_single.pkl', 'wb') as f:
